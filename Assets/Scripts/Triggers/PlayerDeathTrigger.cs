...
public class PlayerDeathTrigger : MonoBehaviour
{
    ...
    private MarioCore playerScript; // Tracks the player inside the trigger.

    private void Update()
    {
        if (playerScript != null)
        {
            //Debug.Log($"{playerScript.State.IsDead}");
            if (playerScript.State.IsDead)
            {
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (other.CompareTag("Player"))
        {
            ...
            playerScript = other.GetComponentInParent<MarioCore>();
        }
    }
    ...
    private void HandlePlayerDeath(MarioCore player)
    {
