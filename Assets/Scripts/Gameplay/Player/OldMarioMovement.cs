...
public class OldMarioMovement : MonoBehaviour
{
    public MarioCore newMario;  // Reference to the new mario object (so we can access its inputs)
    // Edge detection trackers
    private bool _prevRun;
    private bool _prevJump;
    private bool _prevSpin;
    ...
    void Start()
    {
        ...
        colliderY = col.size.y;
        collideroffsetY = col.offset.y;
        ...
        normalSpriteLibrary = GetComponentInChildren<SpriteLibrary>().spriteLibraryAsset;
    ...
    public void EnableAllAbilities(bool enabled)
    {
        if (enabled)
        {
            ...
            Debug.Log("[OldMarioMovement] All Abilities cheat ON");
        }
        else
        {
            RestoreAbilitySnapshot();
            Debug.Log("[OldMarioMovement] All Abilities cheat OFF (restored snapshot)");
        }
    }
    ...
    private void FixedUpdate()
    {
        ...
        if (rb.velocity.y > 0 && doCornerCorrection)
        {
            ...
            float startHeight = col.bounds.size.y / 2 + (rb.velocity.y * Time.fixedDeltaTime) + 0.01f;
            float playerWidth = col.bounds.size.x;
    ...
    bool CheckWall(bool right)
    {
        ...
        if (canWallJump && !onGround && !swimming && (!carrying || canWallJumpWhenHoldingObject))
        {
            ...
            float raycastlength = col.bounds.size.x / 2 + 0.03f;
    ...
    void MoveCharacter(float horizontal)
    {
        ...
        if (crouch && onGround && !carrying && !swimming)
        {
            ...
            inCrouchState = true;
            col.size = new Vector2(col.size.x, crouchColHeight);
            col.offset = new Vector2(col.offset.x, crouchColOffset);
        ...
        else if ((!crouch && onGround) || carrying || groundPounding)
        {
            ...
            inCrouchState = false;
            col.size = new Vector2(col.size.x, colliderY);
            col.offset = new Vector2(col.offset.x, collideroffsetY);
    ...
    private void powerDown()
    {
        ...
        var newOldMarioMovement = transferProperties(newMario);
        PlayerTransformation playerTransformation = newMario.GetComponent<PlayerTransformation>();

        newOldMarioMovement.playDamageSound();
    ...
    public void ChangePowerup(GameObject newMarioObject)
    {
        ...
        var newOldMarioMovement = newMario.GetComponent<OldMarioMovement>();
        var playerTransformation = newMario.GetComponent<PlayerTransformation>();

        // Update the current power-up type
        newOldMarioMovement.currentPowerupType = newMarioObject.GetComponent<OldMarioMovement>().currentPowerupType;
    ...
    public OldMarioMovement transferProperties(GameObject newMario)
    {
        ...
        newOldMarioMovement.FlipTo(facingRight);
        ...
        newOldMarioMovement.pressRunToGrab = pressRunToGrab;
        newOldMarioMovement.crouchToGrab = crouchToGrab;
        newOldMarioMovement.carryMethod = carryMethod;
        newOldMarioMovement.playerNumber = playerNumber;
        newOldMarioMovement.invincetimeremain = invincetimeremain;

        if (starPower)
        {
            newOldMarioMovement.starPower = true;
            newOldMarioMovement.starPowerRemainingTime = starPowerRemainingTime;

            newOldMarioMovement.InvokeRepeating(nameof(changeRainbowColor), 0, 0.1f);

            if (starPowerRemainingTime > 0)
            {
                newOldMarioMovement.Invoke(nameof(stopStarPower), starPowerRemainingTime);
            }
        }
        ...
        if (carrying && heldObjectPosition.transform.childCount > 0)
        {
            ...
            carriedObject.transform.parent = newOldMarioMovement.heldObjectPosition.transform;
            carriedObject.transform.localPosition = Vector3.zero;
            newOldMarioMovement.carrying = true;
        }

        // transfer pressed buttons (for mobile controls)
        newOldMarioMovement.jumpPressed = jumpPressed;
        newOldMarioMovement.runPressed = runPressed;
        newOldMarioMovement.moveInput = moveInput;

        // Set additional abilities to new Mario
        newOldMarioMovement.canCrawl = canCrawl;
        newOldMarioMovement.canWallJump = canWallJump;
        newOldMarioMovement.canWallJumpWhenHoldingObject = canWallJumpWhenHoldingObject;
        newOldMarioMovement.canSpinJump = canSpinJump;
        newOldMarioMovement.canGroundPound = canGroundPound;
        newOldMarioMovement.currentClimbable = currentClimbable;
        newOldMarioMovement.canMidairSpin = canMidairSpin;
        newOldMarioMovement.allowMultipleMidairSpins = allowMultipleMidairSpins;

        // Passing the sound effects
        newOldMarioMovement.yeahAudioClip = yeahAudioClip;

        // Passing ground layer values
        newOldMarioMovement.groundLayer = groundLayer;

        return newOldMarioMovement;
    }
    ...
    private void OnDrawGizmos()
    {
        ...
        float raycastlength = col.bounds.size.y / 2 + 0.03f;
    ...
    // Ghost Input
    private void SyncGhostInput()
    {
        if (newMario == null || newMario.State == null) return;

        // 1. Continuous Axis (Move)
        // We can bypass your old deadzone math completely because 
        // newMario.State.MoveInput is already perfectly filtered!
        moveInput = newMario.State.MoveInput;

        // 2. Run
        bool currentRun = newMario.State.RunPressed;
        if (currentRun && !_prevRun) onRunPressed();
        else if (!currentRun && _prevRun) onRunReleased();
        _prevRun = currentRun;

        // 3. Jump
        bool currentJump = newMario.State.JumpPressed;
        if (currentJump && !_prevJump) onJumpPressed();
        else if (!currentJump && _prevJump) onJumpReleased();
        _prevJump = currentJump;

        // 4. Spin
        // Note: We read SpinHeld instead of SpinPressed from the new state 
        // to capture the raw physical button state, allowing your old script 
        // to run its own midair/ground spin logic accurately.
        bool currentSpin = newMario.State.SpinHeld; 
        if (currentSpin && !_prevSpin) onSpinPressed();
        else if (!currentSpin && _prevSpin) onSpinReleased();
        _prevSpin = currentSpin;

        // 5. Shoot
        bool currentShoot = newMario.State.ShootPressed;
        if (currentShoot && !_prevShoot) onShootPressed();
        else if (!currentShoot && _prevShoot) onShootReleased();
        _prevShoot = currentShoot;

        // Note: ExtraAction and Use aren't standard continuous states in the new State,
        // but if you need them to fire on the old script, you can trigger them similarly
        // by watching for state changes in the new Mario.
    }
